CREATE TABLE IF NOT EXISTS towns (
    `Town` VARCHAR(27) CHARACTER SET utf8,
    `County` VARCHAR(24) CHARACTER SET utf8,
    `Country` VARCHAR(16) CHARACTER SET utf8
);
INSERT INTO towns VALUES
    ('Ampthill','Bedfordshire','England'),
    ('Arlesey','Bedfordshire','England'),
    ('Bedford','Bedfordshire','England'),
    ('Biggleswade','Bedfordshire','England'),
    ('Dunstable','Bedfordshire','England'),
    ('Flitwick','Bedfordshire','England'),
    ('Houghton Regis','Bedfordshire','England'),
    ('Kempston','Bedfordshire','England'),
    ('Leighton Buzzard','Bedfordshire','England'),
    ('Linslade','Bedfordshire','England'),
    ('Luton','Bedfordshire','England'),
    ('Potton','Bedfordshire','England'),
    ('Sandy','Bedfordshire','England'),
    ('Shefford','Bedfordshire','England'),
    ('Stotfold','Bedfordshire','England'),
    ('Wixams','Bedfordshire','England'),
    ('Woburn','Bedfordshire','England'),
    ('Ascot','Berkshire','England'),
    ('Bracknell','Berkshire','England'),
    ('Crowthorne','Berkshire','England'),
    ('Earley','Berkshire','England'),
    ('Eton','Berkshire','England'),
    ('Hungerford','Berkshire','England'),
    ('Lambourn','Berkshire','England'),
    ('Maidenhead','Berkshire','England'),
    ('Newbury','Berkshire','England'),
    ('Reading','Berkshire','England'),
    ('Sandhurst','Berkshire','England'),
    ('Slough','Berkshire','England'),
    ('Thatcham','Berkshire','England'),
    ('Windsor','Berkshire','England'),
    ('Wokingham','Berkshire','England'),
    ('Woodley','Berkshire','England'),
    ('Bristol','Bristol','England'),
    ('Amersham','Buckinghamshire','England'),
    ('Aylesbury','Buckinghamshire','England'),
    ('Beaconsfield','Buckinghamshire','England'),
    ('Bletchley','Buckinghamshire','England'),
    ('Buckingham','Buckinghamshire','England'),
    ('Chesham','Buckinghamshire','England'),
    ('Fenny Stratford','Buckinghamshire','England'),
    ('High Wycombe','Buckinghamshire','England'),
    ('Marlow','Buckinghamshire','England'),
    ('Milton Keynes','Buckinghamshire','England'),
    ('Newport Pagnell','Buckinghamshire','England'),
    ('Olney','Buckinghamshire','England'),
    ('Princes Risborough','Buckinghamshire','England'),
    ('Stony Stratford','Buckinghamshire','England'),
    ('Wendover','Buckinghamshire','England'),
    ('Winslow','Buckinghamshire','England'),
    ('Woburn Sands','Buckinghamshire','England'),
    ('Wolverton and Greenleys','Buckinghamshire','England'),
    ('Cambridge','Cambridgeshire','England'),
    ('Chatteris','Cambridgeshire','England'),
    ('Ely','Cambridgeshire','England'),
    ('Fulbourn','Cambridgeshire','England'),
    ('Godmanchester','Cambridgeshire','England'),
    ('Hanley Grange','Cambridgeshire','England'),
    ('Huntingdon','Cambridgeshire','England'),
    ('March','Cambridgeshire','England'),
    ('Northstowe','Cambridgeshire','England'),
    ('Peterborough','Cambridgeshire','England'),
    ('Ramsey','Cambridgeshire','England'),
    ('Soham','Cambridgeshire','England'),
    ('St Ives','Cambridgeshire','England'),
    ('St Neots','Cambridgeshire','England'),
    ('Whittlesey','Cambridgeshire','England'),
    ('Wisbech','Cambridgeshire','England'),
    ('Alsager','Cheshire','England'),
    ('Birchwood','Cheshire','England'),
    ('Bollington','Cheshire','England'),
    ('Chester','Cheshire','England'),
    ('Congleton','Cheshire','England'),
    ('Crewe','Cheshire','England'),
    ('Ellesmere Port','Cheshire','England'),
    ('Frodsham','Cheshire','England'),
    ('Knutsford','Cheshire','England'),
    ('Macclesfield','Cheshire','England'),
    ('Malpas','Cheshire','England'),
    ('Middlewich','Cheshire','England'),
    ('Nantwich','Cheshire','England'),
    ('Neston','Cheshire','England'),
    ('Northwich','Cheshire','England'),
    ('Poynton with Worth','Cheshire','England'),
    ('Runcorn','Cheshire','England'),
    ('Sandbach','Cheshire','England'),
    ('Warrington','Cheshire','England'),
    ('Widnes','Cheshire','England'),
    ('Wilmslow','Cheshire','England'),
    ('Winsford','Cheshire','England'),
    ('Bodmin','Cornwall','England'),
    ('Bude','Cornwall','England'),
    ('Callington','Cornwall','England'),
    ('Camborne','Cornwall','England'),
    ('Camelford','Cornwall','England'),
    ('Charlestown','Cornwall','England'),
    ('Falmouth','Cornwall','England'),
    ('Fowey','Cornwall','England'),
    ('Hayle','Cornwall','England'),
    ('Helston','Cornwall','England'),
    ('Launceston','Cornwall','England'),
    ('Liskeard','Cornwall','England'),
    ('Looe','Cornwall','England'),
    ('Lostwithiel','Cornwall','England'),
    ('Marazion','Cornwall','England'),
    ('Newlyn','Cornwall','England'),
    ('Newquay','Cornwall','England'),
    ('Padstow','Cornwall','England'),
    ('Par','Cornwall','England'),
    ('Penryn','Cornwall','England'),
    ('Penzance','Cornwall','England'),
    ('Porthleven','Cornwall','England'),
    ('Redruth','Cornwall','England'),
    ('Saltash','Cornwall','England'),
    ('St Austell','Cornwall','England'),
    ('St Blazey','Cornwall','England'),
    ('St Columb Major','Cornwall','England'),
    ('St Ives','Cornwall','England'),
    ('St Just','Cornwall','England'),
    ('St Mawes','Cornwall','England'),
    ('Stratton','Cornwall','England'),
    ('Torpoint','Cornwall','England'),
    ('Truro','Cornwall','England'),
    ('Wadebridge','Cornwall','England'),
    ('Barnard Castle','County Durham','England'),
    ('Billingham','County Durham','England'),
    ('Bishop Auckland','County Durham','England'),
    ('Chester le Street','County Durham','England'),
    ('Consett','County Durham','England'),
    ('Crook','County Durham','England'),
    ('Darlington','County Durham','England'),
    ('Durham','County Durham','England'),
    ('Eaglescliffe','County Durham','England'),
    ('Eastington','County Durham','England'),
    ('Ferryhill','County Durham','England'),
    ('Greater Willington','County Durham','England'),
    ('Hartlepool','County Durham','England'),
    ('Newton Aycliffe','County Durham','England'),
    ('Peterlee','County Durham','England'),
    ('Seaham','County Durham','England'),
    ('Sedgefield','County Durham','England'),
    ('Shildon','County Durham','England'),
    ('Spennymoor','County Durham','England'),
    ('Stanhope','County Durham','England'),
    ('Stanley','County Durham','England'),
    ('Stockton on Tees','County Durham','England'),
    ('Tow Law','County Durham','England'),
    ('Willington','County Durham','England'),
    ('Wolsingham','County Durham','England'),
    ('Alston','Cumbria','England'),
    ('Ambleside','Cumbria','England'),
    ('Appleby in Westmorland','Cumbria','England'),
    ('Aspatria','Cumbria','England'),
    ('Barrow in Furness','Cumbria','England'),
    ('Bowness on Windermere','Cumbria','England'),
    ('Brampton','Cumbria','England'),
    ('Broughton in Furness','Cumbria','England'),
    ('Carlisle','Cumbria','England'),
    ('Cleator Moor','Cumbria','England'),
    ('Cockermouth','Cumbria','England'),
    ('Dalton in Furness','Cumbria','England'),
    ('Egremont','Cumbria','England'),
    ('Grange over Sands','Cumbria','England'),
    ('Harrington','Cumbria','England'),
    ('Kendal','Cumbria','England'),
    ('Keswick','Cumbria','England'),
    ('Kirkby Lonsdale','Cumbria','England'),
    ('Kirkby Stephen','Cumbria','England'),
    ('Longtown','Cumbria','England'),
    ('Maryport','Cumbria','England'),
    ('Millom','Cumbria','England'),
    ('Milnthorpe','Cumbria','England'),
    ('Orgill','Cumbria','England'),
    ('Penrith','Cumbria','England'),
    ('Sedbergh','Cumbria','England'),
    ('Silloth','Cumbria','England'),
    ('Staveley','Cumbria','England'),
    ('Ulverston','Cumbria','England'),
    ('Whitehaven','Cumbria','England'),
    ('Wigton','Cumbria','England'),
    ('Windermere','Cumbria','England'),
    ('Workington','Cumbria','England'),
    ('Alfreton','Derbyshire','England'),
    ('Ashbourne','Derbyshire','England'),
    ('Bakewell','Derbyshire','England'),
    ('Barrow Hill and Whittington','Derbyshire','England'),
    ('Belper','Derbyshire','England'),
    ('Bolsover','Derbyshire','England'),
    ('Buxton','Derbyshire','England'),
    ('Chapel en le Frith','Derbyshire','England'),
    ('Chesterfield','Derbyshire','England'),
    ('Clay Cross','Derbyshire','England'),
    ('Darley Dale','Derbyshire','England'),
    ('Derby','Derbyshire','England'),
    ('Dronfield','Derbyshire','England'),
    ('Dronfield Woodhouse','Derbyshire','England'),
    ('Eckington','Derbyshire','England'),
    ('Fairfield','Derbyshire','England'),
    ('Glossop','Derbyshire','England'),
    ('Hadfield','Derbyshire','England'),
    ('Heanor','Derbyshire','England'),
    ('Ilkeston','Derbyshire','England'),
    ('Killamarsh','Derbyshire','England'),
    ('Langley Mill','Derbyshire','England'),
    ('Long Eaton','Derbyshire','England'),
    ('Matlock','Derbyshire','England'),
    ('Melbourne','Derbyshire','England'),
    ('Netherthorpe','Derbyshire','England'),
    ('New Mills','Derbyshire','England'),
    ('Over Woodhouse','Derbyshire','England'),
    ('Ripley','Derbyshire','England'),
    ('Sandiacre','Derbyshire','England'),
    ('Shallcross','Derbyshire','England'),
    ('Shirebrook','Derbyshire','England'),
    ('Staveley','Derbyshire','England'),
    ('Swadlincote','Derbyshire','England'),
    ('Whaley Bridge','Derbyshire','England'),
    ('Wirksworth','Derbyshire','England'),
    ('Ashburton','Devon','England'),
    ('Axminster','Devon','England'),
    ('Bampton','Devon','England'),
    ('Barnstaple','Devon','England'),
    ('Bideford','Devon','England'),
    ('Bovey Tracey','Devon','England'),
    ('Bradninch','Devon','England'),
    ('Brixham','Devon','England'),
    ('Buckfastleigh','Devon','England'),
    ('Budleigh Salterton','Devon','England'),
    ('Chagford','Devon','England'),
    ('Chudleigh','Devon','England'),
    ('Chulmleigh','Devon','England'),
    ('Colyton','Devon','England'),
    ('Crediton','Devon','England'),
    ('Cullompton','Devon','England'),
    ('Dartmouth','Devon','England'),
    ('Dawlish','Devon','England'),
    ('Exeter','Devon','England'),
    ('Exmouth','Devon','England'),
    ('Great Torrington','Devon','England'),
    ('Hartland','Devon','England'),
    ('Hatherleigh','Devon','England'),
    ('Highampton','Devon','England'),
    ('Holsworthy','Devon','England'),
    ('Honiton','Devon','England'),
    ('Ilfracombe','Devon','England'),
    ('Ivybridge','Devon','England'),
    ('Kingsbridge','Devon','England'),
    ('Kingsteignton','Devon','England'),
    ('Lynton','Devon','England'),
    ('Modbury','Devon','England');
INSERT INTO towns VALUES
    ('Moretonhampstead','Devon','England'),
    ('Newton Abbot','Devon','England'),
    ('North Tawton','Devon','England'),
    ('Northam','Devon','England'),
    ('Okehampton','Devon','England'),
    ('Ottery St Mary','Devon','England'),
    ('Paignton','Devon','England'),
    ('Plymouth','Devon','England'),
    ('Princetown','Devon','England'),
    ('Salcombe','Devon','England'),
    ('Seaton','Devon','England'),
    ('Sherford','Devon','England'),
    ('Sidmouth','Devon','England'),
    ('South Molton','Devon','England'),
    ('Tavistock','Devon','England'),
    ('Teignmouth','Devon','England'),
    ('Tiverton','Devon','England'),
    ('Topsham','Devon','England'),
    ('Torquay','Devon','England'),
    ('Totnes','Devon','England'),
    ('Beaminster','Dorset','England'),
    ('Blandford Forum','Dorset','England'),
    ('Bournemouth','Dorset','England'),
    ('Bridport','Dorset','England'),
    ('Chickerell','Dorset','England'),
    ('Christchurch','Dorset','England'),
    ('Dorchester','Dorset','England'),
    ('Ferndown','Dorset','England'),
    ('Gillingham','Dorset','England'),
    ('Highcliffe','Dorset','England'),
    ('Lyme Regis','Dorset','England'),
    ('Poole','Dorset','England'),
    ('Portland','Dorset','England'),
    ('Shaftesbury','Dorset','England'),
    ('Sherborne','Dorset','England'),
    ('Stalbridge','Dorset','England'),
    ('Sturminster Newton','Dorset','England'),
    ('Swanage','Dorset','England'),
    ('Verwood','Dorset','England'),
    ('Wareham','Dorset','England'),
    ('Weymouth','Dorset','England'),
    ('Wimborne Minster','Dorset','England'),
    ('Beverley','East Riding of Yorkshire','England'),
    ('Bridlington','East Riding of Yorkshire','England'),
    ('Brough','East Riding of Yorkshire','England'),
    ('Driffield','East Riding of Yorkshire','England'),
    ('Goole','East Riding of Yorkshire','England'),
    ('Hedon','East Riding of Yorkshire','England'),
    ('Hessle','East Riding of Yorkshire','England'),
    ('Hornsea','East Riding of Yorkshire','England'),
    ('Howden','East Riding of Yorkshire','England'),
    ('Market Weighton','East Riding of Yorkshire','England'),
    ('Pocklington','East Riding of Yorkshire','England'),
    ('Snaith','East Riding of Yorkshire','England'),
    ('South Cave','East Riding of Yorkshire','England'),
    ('Withernsea','East Riding of Yorkshire','England'),
    ('Battle','East Sussex','England'),
    ('Bexhill on Sea','East Sussex','England'),
    ('Brighton','East Sussex','England'),
    ('Crowborough','East Sussex','England'),
    ('Eastbourne','East Sussex','England'),
    ('Hailsham','East Sussex','England'),
    ('Hastings','East Sussex','England'),
    ('Heathfield','East Sussex','England'),
    ('Hove','East Sussex','England'),
    ('Lewes','East Sussex','England'),
    ('Newhaven','East Sussex','England'),
    ('Ore Valley','East Sussex','England'),
    ('Peacehaven','East Sussex','England'),
    ('Polegate','East Sussex','England'),
    ('Rye','East Sussex','England'),
    ('Seaford','East Sussex','England'),
    ('Telscombe','East Sussex','England'),
    ('Uckfield','East Sussex','England'),
    ('Wadhurst','East Sussex','England'),
    ('Winchelsea','East Sussex','England'),
    ('Basildon','Essex','England'),
    ('Billericay','Essex','England'),
    ('Braintree','Essex','England'),
    ('Brentwood','Essex','England'),
    ('Brightlingsea','Essex','England'),
    ('Buckhurst Hill','Essex','England'),
    ('Burnham on Crouch','Essex','England'),
    ('Canvey Island','Essex','England'),
    ('Chafford Hundred','Essex','England'),
    ('Chelmsford','Essex','England'),
    ('Clackwell','Essex','England'),
    ('Clacton on Sea','Essex','England'),
    ('Coggeshall','Essex','England'),
    ('Colchester','Essex','England'),
    ('Corringham','Essex','England'),
    ('Dovercourt','Essex','England'),
    ('Eastwood','Essex','England'),
    ('Epping','Essex','England'),
    ('Frinton on Sea','Essex','England'),
    ('Grays','Essex','England'),
    ('Great Dunmow','Essex','England'),
    ('Hadleigh','Essex','England'),
    ('Halstead','Essex','England'),
    ('Harlow','Essex','England'),
    ('Harwich','Essex','England'),
    ('Heybridge','Essex','England'),
    ('Hockley','Essex','England'),
    ('Holland on Sea','Essex','England'),
    ('Ingatestone','Essex','England'),
    ('Laindon','Essex','England'),
    ('Langdon Hills','Essex','England'),
    ('Leigh on Sea','Essex','England'),
    ('Loughton','Essex','England'),
    ('Maldon','Essex','England'),
    ('Manningtree','Essex','England'),
    ('North Shoebury','Essex','England'),
    ('Ongar','Essex','England'),
    ('Parkeston','Essex','England'),
    ('Pitsea','Essex','England'),
    ('Prettlewell','Essex','England'),
    ('Rayleigh','Essex','England'),
    ('Rochford','Essex','England'),
    ('Romford','Essex','England'),
    ('Saffron Walden','Essex','England'),
    ('Shoeburyness','Essex','England'),
    ('South Benfleet','Essex','England'),
    ('South Woodham Ferrers','Essex','England'),
    ('Southchurch','Essex','England'),
    ('Southend on Sea','Essex','England'),
    ('Southminster','Essex','England'),
    ('Stanfield le Hope','Essex','England'),
    ('Thaxted','Essex','England'),
    ('Thorpe Bay','Essex','England'),
    ('Tilbury','Essex','England'),
    ('Waltham Abbey','Essex','England'),
    ('Walton on the Naze','Essex','England'),
    ('West Mersea','Essex','England'),
    ('West Thurrock','Essex','England'),
    ('West Tilbury','Essex','England'),
    ('Westcliff on Sea','Essex','England'),
    ('Wickford','Essex','England'),
    ('Witham','Essex','England'),
    ('Wivenhoe','Essex','England'),
    ('Berkeley','Gloucestershire','England'),
    ('Bradley Stoke','Gloucestershire','England'),
    ('Cheltenham','Gloucestershire','England'),
    ('Chipping Campden','Gloucestershire','England'),
    ('Chipping Sodbury','Gloucestershire','England'),
    ('Cinderford','Gloucestershire','England'),
    ('Cirencester','Gloucestershire','England'),
    ('Coleford','Gloucestershire','England'),
    ('Dursley','Gloucestershire','England'),
    ('Fairford','Gloucestershire','England'),
    ('Filton','Gloucestershire','England'),
    ('Gloucester','Gloucestershire','England'),
    ('Kingswood','Gloucestershire','England'),
    ('Lechlade','Gloucestershire','England'),
    ('Lydney','Gloucestershire','England'),
    ('Minchinhampton','Gloucestershire','England'),
    ('Mitcheldean','Gloucestershire','England'),
    ('Moreton in Marsh','Gloucestershire','England'),
    ('Nailsworth','Gloucestershire','England'),
    ('Newent','Gloucestershire','England'),
    ('Northleach','Gloucestershire','England'),
    ('Painswick','Gloucestershire','England'),
    ('Patchway','Gloucestershire','England'),
    ('Stonehouse','Gloucestershire','England'),
    ('Stow on the Wold','Gloucestershire','England'),
    ('Stroud','Gloucestershire','England'),
    ('Tetbury','Gloucestershire','England'),
    ('Tewkesbury','Gloucestershire','England'),
    ('Thornbury','Gloucestershire','England'),
    ('Wickwar','Gloucestershire','England'),
    ('Winchcombe','Gloucestershire','England'),
    ('Wotton under Edge','Gloucestershire','England'),
    ('Yate','Gloucestershire','England'),
    ('Acton','Greater London','England'),
    ('Barking','Greater London','England'),
    ('Barnes','Greater London','England'),
    ('Beckenham','Greater London','England'),
    ('Bexley','Greater London','England'),
    ('Brentford','Greater London','England'),
    ('Bromley','Greater London','England'),
    ('Chingford','Greater London','England'),
    ('Croydon','Greater London','England'),
    ('Dagenham','Greater London','England'),
    ('Ealing','Greater London','England'),
    ('East Ham','Greater London','England'),
    ('Edgware','Greater London','England'),
    ('Edmonton','Greater London','England'),
    ('Enfield','Greater London','England'),
    ('Erith','Greater London','England'),
    ('Finchley','Greater London','England'),
    ('Harrow','Greater London','England'),
    ('Hendon','Greater London','England'),
    ('Hornsey','Greater London','England'),
    ('Ilford','Greater London','England'),
    ('Kingston upon Thames','Greater London','England'),
    ('Leyton','Greater London','England'),
    ('Mitcham','Greater London','England'),
    ('Richmond','Greater London','England'),
    ('Southall','Greater London','England'),
    ('Southgate','Greater London','England'),
    ('St Mary Cray','Greater London','England'),
    ('Surbiton','Greater London','England'),
    ('Tottenham','Greater London','England'),
    ('Twickenham','Greater London','England'),
    ('Uxbridge','Greater London','England'),
    ('Walthamstow','Greater London','England'),
    ('Wembley','Greater London','England'),
    ('West Ham','Greater London','England'),
    ('Willesden','Greater London','England'),
    ('Wimbledon','Greater London','England'),
    ('Wood Green','Greater London','England'),
    ('Altrincham','Greater Manchester','England'),
    ('Ashton in Makerfield','Greater Manchester','England'),
    ('Ashton under Lyne','Greater Manchester','England'),
    ('Atherton','Greater Manchester','England'),
    ('Audenshaw','Greater Manchester','England'),
    ('Blackrod','Greater Manchester','England'),
    ('Bolton','Greater Manchester','England'),
    ('Bury','Greater Manchester','England'),
    ('Cadishead','Greater Manchester','England'),
    ('Chadderton','Greater Manchester','England'),
    ('Cheadle','Greater Manchester','England'),
    ('Cheadle Hulme','Greater Manchester','England'),
    ('Denton','Greater Manchester','England'),
    ('Droylsden','Greater Manchester','England'),
    ('Dukinfield','Greater Manchester','England'),
    ('Eccles','Greater Manchester','England'),
    ('Failsworth','Greater Manchester','England'),
    ('Farnworth','Greater Manchester','England'),
    ('Golbourne','Greater Manchester','England'),
    ('Heywood','Greater Manchester','England'),
    ('Hindley','Greater Manchester','England'),
    ('Horwich','Greater Manchester','England'),
    ('Hyde','Greater Manchester','England'),
    ('Ince in Makerfield','Greater Manchester','England'),
    ('Irlam','Greater Manchester','England'),
    ('Kearsley','Greater Manchester','England'),
    ('Leigh','Greater Manchester','England'),
    ('Littleborough','Greater Manchester','England'),
    ('Manchester','Greater Manchester','England'),
    ('Middleton','Greater Manchester','England'),
    ('Milnrow','Greater Manchester','England'),
    ('Mossley','Greater Manchester','England'),
    ('Oldham','Greater Manchester','England'),
    ('Partington','Greater Manchester','England'),
    ('Pendlebury','Greater Manchester','England'),
    ('Prestwich','Greater Manchester','England'),
    ('Radcliffe','Greater Manchester','England'),
    ('Ramsbottom','Greater Manchester','England'),
    ('Rochdale','Greater Manchester','England'),
    ('Royton','Greater Manchester','England');
INSERT INTO towns VALUES
    ('Sale','Greater Manchester','England'),
    ('Salford','Greater Manchester','England'),
    ('Shaw and Crompton','Greater Manchester','England'),
    ('Stalybridge','Greater Manchester','England'),
    ('Stockport','Greater Manchester','England'),
    ('Stretford','Greater Manchester','England'),
    ('Swinton','Greater Manchester','England'),
    ('Tottington','Greater Manchester','England'),
    ('Tyldsley','Greater Manchester','England'),
    ('Walkden','Greater Manchester','England'),
    ('Westhoughton','Greater Manchester','England'),
    ('Whitefield','Greater Manchester','England'),
    ('Wigan','Greater Manchester','England'),
    ('Worsley','Greater Manchester','England'),
    ('Aldershot','Hampshire','England'),
    ('Alton','Hampshire','England'),
    ('Andover','Hampshire','England'),
    ('Basingstoke','Hampshire','England'),
    ('Bishop''s Waltham','Hampshire','England'),
    ('Blackwater and Hawley','Hampshire','England'),
    ('Bordon','Hampshire','England'),
    ('Eastleigh','Hampshire','England'),
    ('Emsworth','Hampshire','England'),
    ('Fareham','Hampshire','England'),
    ('Farnborough','Hampshire','England'),
    ('Fleet','Hampshire','England'),
    ('Fordingbridge','Hampshire','England'),
    ('Gosport','Hampshire','England'),
    ('Havant','Hampshire','England'),
    ('Hedge End','Hampshire','England'),
    ('Hythe','Hampshire','England'),
    ('Lee on the Solent','Hampshire','England'),
    ('Lymington','Hampshire','England'),
    ('Lyndhurst','Hampshire','England'),
    ('New Alresford','Hampshire','England'),
    ('New Milton','Hampshire','England'),
    ('North Camp','Hampshire','England'),
    ('Petersfield','Hampshire','England'),
    ('Portchester','Hampshire','England'),
    ('Portsmouth','Hampshire','England'),
    ('Ringwood','Hampshire','England'),
    ('Romsey','Hampshire','England'),
    ('Southampton','Hampshire','England'),
    ('Southsea','Hampshire','England'),
    ('Southwick','Hampshire','England'),
    ('Tadley','Hampshire','England'),
    ('Totton','Hampshire','England'),
    ('Waterlooville','Hampshire','England'),
    ('Whitchurch','Hampshire','England'),
    ('Whitehill','Hampshire','England'),
    ('Wickham','Hampshire','England'),
    ('Winchester','Hampshire','England'),
    ('Yateley','Hampshire','England'),
    ('Bromyard','Herefordshire','England'),
    ('Hatfield','Herefordshire','England'),
    ('Hereford','Herefordshire','England'),
    ('Kington','Herefordshire','England'),
    ('Ledbury','Herefordshire','England'),
    ('Leominster','Herefordshire','England'),
    ('Longtown','Herefordshire','England'),
    ('Ross on Wye','Herefordshire','England'),
    ('Baldock','Hertfordshire','England'),
    ('Barnet','Hertfordshire','England'),
    ('Berkhamsted','Hertfordshire','England'),
    ('Bishop''s Stortford','Hertfordshire','England'),
    ('Borehamwood','Hertfordshire','England'),
    ('Broxbourne','Hertfordshire','England'),
    ('Buntingford','Hertfordshire','England'),
    ('Bushey','Hertfordshire','England'),
    ('Cheshunt','Hertfordshire','England'),
    ('Chorleywood','Hertfordshire','England'),
    ('Elstree','Hertfordshire','England'),
    ('Harpenden','Hertfordshire','England'),
    ('Hatfield','Hertfordshire','England'),
    ('Hemel Hempstead','Hertfordshire','England'),
    ('Hertford','Hertfordshire','England'),
    ('Hitchin','Hertfordshire','England'),
    ('Hoddesdon','Hertfordshire','England'),
    ('Letchworth','Hertfordshire','England'),
    ('Potters Bar','Hertfordshire','England'),
    ('Rickmansworth','Hertfordshire','England'),
    ('Royston','Hertfordshire','England'),
    ('Sawbridgeworth','Hertfordshire','England'),
    ('Stevenage','Hertfordshire','England'),
    ('Tring','Hertfordshire','England'),
    ('Waltham Cross','Hertfordshire','England'),
    ('Ware','Hertfordshire','England'),
    ('Watford','Hertfordshire','England'),
    ('Welwyn Garden City','Hertfordshire','England'),
    ('Appley','Isle of Wight','England'),
    ('Brading','Isle of Wight','England'),
    ('Cowes','Isle of Wight','England'),
    ('East Cowes','Isle of Wight','England'),
    ('Newport','Isle of Wight','England'),
    ('Ryde','Isle of Wight','England'),
    ('Sandown','Isle of Wight','England'),
    ('Shanklin','Isle of Wight','England'),
    ('Ventnor','Isle of Wight','England'),
    ('Yarmouth','Isle of Wight','England'),
    ('Ashford','Kent','England'),
    ('Broadstairs','Kent','England'),
    ('Canterbury','Kent','England'),
    ('Chatham','Kent','England'),
    ('Cranbrook','Kent','England'),
    ('Crayford','Kent','England'),
    ('Dartford','Kent','England'),
    ('Deal','Kent','England'),
    ('Dover','Kent','England'),
    ('Edenbridge','Kent','England'),
    ('Faversham','Kent','England'),
    ('Folkestone','Kent','England'),
    ('Fordwich','Kent','England'),
    ('Gillingham','Kent','England'),
    ('Gravesend','Kent','England'),
    ('Greenhill','Kent','England'),
    ('Herne Bay','Kent','England'),
    ('Hythe','Kent','England'),
    ('Lydd','Kent','England'),
    ('Maidstone','Kent','England'),
    ('Margate','Kent','England'),
    ('Minster','Kent','England'),
    ('New Romney','Kent','England'),
    ('Northfleet','Kent','England'),
    ('Orpington','Kent','England'),
    ('Paddock Wood','Kent','England'),
    ('Queenborough','Kent','England'),
    ('Rainham','Kent','England'),
    ('Ramsgate','Kent','England'),
    ('Rochester','Kent','England'),
    ('Royal Tunbridge Wells','Kent','England'),
    ('Sandwich','Kent','England'),
    ('Sevenoaks','Kent','England'),
    ('Sheerness','Kent','England'),
    ('Sittingbourne','Kent','England'),
    ('Snodland','Kent','England'),
    ('Southborough','Kent','England'),
    ('Strood','Kent','England'),
    ('Swanley','Kent','England'),
    ('Swanscombe and Greenhithe','Kent','England'),
    ('Tenterden','Kent','England'),
    ('Tonbridge','Kent','England'),
    ('Tunbridge Wells','Kent','England'),
    ('West Malling','Kent','England'),
    ('Westerham','Kent','England'),
    ('Westgate on Sea','Kent','England'),
    ('Whitstable','Kent','England'),
    ('Accrington','Lancashire','England'),
    ('Adlington','Lancashire','England'),
    ('Bacup','Lancashire','England'),
    ('Barnoldswick','Lancashire','England'),
    ('Blackburn','Lancashire','England'),
    ('Blackpool','Lancashire','England'),
    ('Brierfield','Lancashire','England'),
    ('Burnley','Lancashire','England'),
    ('Carnforth','Lancashire','England'),
    ('Chorley','Lancashire','England'),
    ('Clayton le Moors','Lancashire','England'),
    ('Cleveleys','Lancashire','England'),
    ('Clitheroe','Lancashire','England'),
    ('Colne','Lancashire','England'),
    ('Darwen','Lancashire','England'),
    ('Failsworth','Lancashire','England'),
    ('Fleetwood','Lancashire','England'),
    ('Garstang','Lancashire','England'),
    ('Great Harwood','Lancashire','England'),
    ('Haslingden','Lancashire','England'),
    ('Kirkham','Lancashire','England'),
    ('Lancaster','Lancashire','England'),
    ('Leyland','Lancashire','England'),
    ('Longridge','Lancashire','England'),
    ('Lytham St Annes','Lancashire','England'),
    ('Medlar with Wesham','Lancashire','England'),
    ('Morecambe','Lancashire','England'),
    ('Nelson','Lancashire','England'),
    ('Ormskirk','Lancashire','England'),
    ('Oswaldtwistle','Lancashire','England'),
    ('Padiham','Lancashire','England'),
    ('Penwortham','Lancashire','England'),
    ('Poulton le Fylde','Lancashire','England'),
    ('Preesall','Lancashire','England'),
    ('Preston','Lancashire','England'),
    ('Rawtenstall','Lancashire','England'),
    ('Skelmersdale','Lancashire','England'),
    ('Thornton','Lancashire','England'),
    ('Thornton Cleveleys','Lancashire','England'),
    ('Wesham','Lancashire','England'),
    ('Whitworth','Lancashire','England'),
    ('Ashby de la Zouch','Leicestershire','England'),
    ('Ashby Woulds','Leicestershire','England'),
    ('Braunstone Town','Leicestershire','England'),
    ('Coalville','Leicestershire','England'),
    ('Earl Shilton','Leicestershire','England'),
    ('Hinckley','Leicestershire','England'),
    ('Leicester','Leicestershire','England'),
    ('Loughborough','Leicestershire','England'),
    ('Lutterworth','Leicestershire','England'),
    ('Market Bosworth','Leicestershire','England'),
    ('Market Harborough','Leicestershire','England'),
    ('Melton Mowbray','Leicestershire','England'),
    ('Oadby','Leicestershire','England'),
    ('Shepshed','Leicestershire','England'),
    ('Syston','Leicestershire','England'),
    ('Wigston Magna','Leicestershire','England'),
    ('Alford','Lincolnshire','England'),
    ('Barton upon Humber','Lincolnshire','England'),
    ('Boston','Lincolnshire','England'),
    ('Bottesford','Lincolnshire','England'),
    ('Bourne','Lincolnshire','England'),
    ('Brigg','Lincolnshire','England'),
    ('Broughton','Lincolnshire','England'),
    ('Burgh le Marsh','Lincolnshire','England'),
    ('Caistor','Lincolnshire','England'),
    ('Cleethorpes','Lincolnshire','England'),
    ('Coningsby','Lincolnshire','England'),
    ('Crowland','Lincolnshire','England'),
    ('Crowle','Lincolnshire','England'),
    ('Epworth','Lincolnshire','England'),
    ('Gainsborough','Lincolnshire','England'),
    ('Grantham','Lincolnshire','England'),
    ('Grimsby','Lincolnshire','England'),
    ('Holbeach','Lincolnshire','England'),
    ('Horncastle','Lincolnshire','England'),
    ('Immingham','Lincolnshire','England'),
    ('Kirton in Lindsey','Lincolnshire','England'),
    ('Lincoln','Lincolnshire','England'),
    ('Little Coates','Lincolnshire','England'),
    ('Long Sutton','Lincolnshire','England'),
    ('Louth','Lincolnshire','England'),
    ('Mablethorpe','Lincolnshire','England'),
    ('Mablethorpe and Sutton','Lincolnshire','England'),
    ('Market Deeping','Lincolnshire','England'),
    ('Market Rasen','Lincolnshire','England'),
    ('North Hykeham','Lincolnshire','England'),
    ('Scunthorpe','Lincolnshire','England'),
    ('Skegness','Lincolnshire','England'),
    ('Sleaford','Lincolnshire','England'),
    ('Spalding','Lincolnshire','England'),
    ('Spilsby','Lincolnshire','England'),
    ('Stamford','Lincolnshire','England'),
    ('The Deepings','Lincolnshire','England'),
    ('Wainfleet','Lincolnshire','England'),
    ('Winterton','Lincolnshire','England'),
    ('Wragby','Lincolnshire','England'),
    ('Bebington','Merseyside','England'),
    ('Birkenhead','Merseyside','England'),
    ('Bootle','Merseyside','England'),
    ('Bromborough','Merseyside','England'),
    ('Crosby','Merseyside','England'),
    ('Earlestown','Merseyside','England'),
    ('Formby','Merseyside','England');
INSERT INTO towns VALUES
    ('Halewood','Merseyside','England'),
    ('Heswall','Merseyside','England'),
    ('Hoylake','Merseyside','England'),
    ('Huyton','Merseyside','England'),
    ('Kirkby','Merseyside','England'),
    ('Liverpool','Merseyside','England'),
    ('Maghull','Merseyside','England'),
    ('Newton le Willows','Merseyside','England'),
    ('Prescot','Merseyside','England'),
    ('Rainford','Merseyside','England'),
    ('Rainhill','Merseyside','England'),
    ('Southport','Merseyside','England'),
    ('St Helens','Merseyside','England'),
    ('Wallasey','Merseyside','England'),
    ('Whiston','Merseyside','England'),
    ('Acle','Norfolk','England'),
    ('Attleborough','Norfolk','England'),
    ('Aylsham','Norfolk','England'),
    ('Caister on Sea','Norfolk','England'),
    ('Cromer','Norfolk','England'),
    ('Dereham','Norfolk','England'),
    ('Diss','Norfolk','England'),
    ('Downham Market','Norfolk','England'),
    ('Fakenham','Norfolk','England'),
    ('Gorleston','Norfolk','England'),
    ('Great Yarmouth','Norfolk','England'),
    ('Hingham','Norfolk','England'),
    ('Holt','Norfolk','England'),
    ('Hunstanton','Norfolk','England'),
    ('King''s Lynn','Norfolk','England'),
    ('Loddon','Norfolk','England'),
    ('North Walsham','Norfolk','England'),
    ('Norwich','Norfolk','England'),
    ('Rackheath','Norfolk','England'),
    ('Redenhall with Harleston','Norfolk','England'),
    ('Reepham','Norfolk','England'),
    ('Sheringham','Norfolk','England'),
    ('Stalham','Norfolk','England'),
    ('Swaffham','Norfolk','England'),
    ('Thetford','Norfolk','England'),
    ('Thorpe St Andrew','Norfolk','England'),
    ('Watton','Norfolk','England'),
    ('Wells next the Sea','Norfolk','England'),
    ('Wroxham','Norfolk','England'),
    ('Wymondham','Norfolk','England'),
    ('Bedale','North Yorkshire','England'),
    ('Bentham','North Yorkshire','England'),
    ('Boroughbridge','North Yorkshire','England'),
    ('Colburn','North Yorkshire','England'),
    ('Easingwold','North Yorkshire','England'),
    ('Eston','North Yorkshire','England'),
    ('Filey','North Yorkshire','England'),
    ('Grangetown','North Yorkshire','England'),
    ('Grassington','North Yorkshire','England'),
    ('Guisborough','North Yorkshire','England'),
    ('Harrogate','North Yorkshire','England'),
    ('Hawes','North Yorkshire','England'),
    ('Haxby','North Yorkshire','England'),
    ('Helmsley','North Yorkshire','England'),
    ('Ingleby Barwick','North Yorkshire','England'),
    ('Kirkbymoorside','North Yorkshire','England'),
    ('Knaresborough','North Yorkshire','England'),
    ('Leyburn','North Yorkshire','England'),
    ('Loftus','North Yorkshire','England'),
    ('Malton','North Yorkshire','England'),
    ('Masham','North Yorkshire','England'),
    ('Middleham','North Yorkshire','England'),
    ('Middlesbrough','North Yorkshire','England'),
    ('Northallerton','North Yorkshire','England'),
    ('Norton on Derwent','North Yorkshire','England'),
    ('Pateley Bridge','North Yorkshire','England'),
    ('Pickering','North Yorkshire','England'),
    ('Redcar','North Yorkshire','England'),
    ('Richmond','North Yorkshire','England'),
    ('Ripon','North Yorkshire','England'),
    ('Robin Hood''s Bay','North Yorkshire','England'),
    ('Saltburn by the Sea','North Yorkshire','England'),
    ('Scarborough','North Yorkshire','England'),
    ('Selby','North Yorkshire','England'),
    ('Settle','North Yorkshire','England'),
    ('Sherburn in Elmet','North Yorkshire','England'),
    ('Skelton in Cleveland','North Yorkshire','England'),
    ('Skipton','North Yorkshire','England'),
    ('Stockton on Tees','North Yorkshire','England'),
    ('Stokesley','North Yorkshire','England'),
    ('Tadcaster','North Yorkshire','England'),
    ('Thirsk','North Yorkshire','England'),
    ('Thornaby on Tees','North Yorkshire','England'),
    ('Whitby','North Yorkshire','England'),
    ('Yarm','North Yorkshire','England'),
    ('York','North Yorkshire','England'),
    ('Brackley','Northamptonshire','England'),
    ('Burton Latimer','Northamptonshire','England'),
    ('Corby','Northamptonshire','England'),
    ('Daventry','Northamptonshire','England'),
    ('Desborough','Northamptonshire','England'),
    ('Higham Ferrers','Northamptonshire','England'),
    ('Irthlingborough','Northamptonshire','England'),
    ('Kettering','Northamptonshire','England'),
    ('Northampton','Northamptonshire','England'),
    ('Oundle','Northamptonshire','England'),
    ('Raunds','Northamptonshire','England'),
    ('Rothwell','Northamptonshire','England'),
    ('Rushden','Northamptonshire','England'),
    ('Thrapston','Northamptonshire','England'),
    ('Towcester','Northamptonshire','England'),
    ('Wellingborough','Northamptonshire','England'),
    ('Alnwick','Northumberland','England'),
    ('Amble','Northumberland','England'),
    ('Ashington','Northumberland','England'),
    ('Bedlington','Northumberland','England'),
    ('Berwick upon Tweed','Northumberland','England'),
    ('Blyth','Northumberland','England'),
    ('Corbridge','Northumberland','England'),
    ('Cramlington','Northumberland','England'),
    ('Haltwhistle','Northumberland','England'),
    ('Hexham','Northumberland','England'),
    ('Morpeth','Northumberland','England'),
    ('Newbiggin by the Sea','Northumberland','England'),
    ('Ponteland','Northumberland','England'),
    ('Prudhoe','Northumberland','England'),
    ('Rothbury','Northumberland','England'),
    ('West Bedlington','Northumberland','England'),
    ('Wooler','Northumberland','England'),
    ('Arnold','Nottinghamshire','England'),
    ('Beeston','Nottinghamshire','England'),
    ('Bingham','Nottinghamshire','England'),
    ('Bracebridge','Nottinghamshire','England'),
    ('Bulwell','Nottinghamshire','England'),
    ('Carlton','Nottinghamshire','England'),
    ('Cotgrave','Nottinghamshire','England'),
    ('East Retford','Nottinghamshire','England'),
    ('Eastwood','Nottinghamshire','England'),
    ('Harworth and Bircotes','Nottinghamshire','England'),
    ('Hucknall','Nottinghamshire','England'),
    ('Kilton','Nottinghamshire','England'),
    ('Kimberley','Nottinghamshire','England'),
    ('Kirkby in Ashfield','Nottinghamshire','England'),
    ('Mansfield','Nottinghamshire','England'),
    ('Netherfield','Nottinghamshire','England'),
    ('Newark on Trent','Nottinghamshire','England'),
    ('Nottingham','Nottinghamshire','England'),
    ('Ollerton','Nottinghamshire','England'),
    ('Ollerton and Boughton','Nottinghamshire','England'),
    ('Retford','Nottinghamshire','England'),
    ('Southwell','Nottinghamshire','England'),
    ('Stapleford','Nottinghamshire','England'),
    ('Sutton in Ashfield','Nottinghamshire','England'),
    ('Warsop','Nottinghamshire','England'),
    ('West Bridgefield','Nottinghamshire','England'),
    ('Worksop','Nottinghamshire','England'),
    ('Abingdon','Oxfordshire','England'),
    ('Banbury','Oxfordshire','England'),
    ('Bicester','Oxfordshire','England'),
    ('Burford','Oxfordshire','England'),
    ('Carterton','Oxfordshire','England'),
    ('Charlbury','Oxfordshire','England'),
    ('Chipping Norton','Oxfordshire','England'),
    ('Didcot','Oxfordshire','England'),
    ('Dorchester','Oxfordshire','England'),
    ('Faringdon','Oxfordshire','England'),
    ('Henley on Thames','Oxfordshire','England'),
    ('Neithrop','Oxfordshire','England'),
    ('Oxford','Oxfordshire','England'),
    ('Ruscote','Oxfordshire','England'),
    ('Thame','Oxfordshire','England'),
    ('Wallingford','Oxfordshire','England'),
    ('Wantage','Oxfordshire','England'),
    ('Watlington','Oxfordshire','England'),
    ('Weston Otmoor','Oxfordshire','England'),
    ('Witney','Oxfordshire','England'),
    ('Woodstock','Oxfordshire','England'),
    ('Oakham','Rutland','England'),
    ('Uppingham','Rutland','England'),
    ('Bishop''s Castle','Shropshire','England'),
    ('Bridgnorth','Shropshire','England'),
    ('Broseley','Shropshire','England'),
    ('Church Stretton','Shropshire','England'),
    ('Cleobury Mortimer','Shropshire','England'),
    ('Clun','Shropshire','England'),
    ('Craven Arms','Shropshire','England'),
    ('Dawley','Shropshire','England'),
    ('Ellesmere','Shropshire','England'),
    ('Ludlow','Shropshire','England'),
    ('Madeley','Shropshire','England'),
    ('Market Drayton','Shropshire','England'),
    ('Much Wenlock','Shropshire','England'),
    ('Newport','Shropshire','England'),
    ('Oakengates','Shropshire','England'),
    ('Oswestry','Shropshire','England'),
    ('Shifnal','Shropshire','England'),
    ('Shrewsbury','Shropshire','England'),
    ('Telford','Shropshire','England'),
    ('Wellington','Shropshire','England'),
    ('Wem','Shropshire','England'),
    ('Whitchurch','Shropshire','England'),
    ('Axbridge','Somerset','England'),
    ('Bath','Somerset','England'),
    ('Bridgwater','Somerset','England'),
    ('Bruton','Somerset','England'),
    ('Burnham on Sea','Somerset','England'),
    ('Castle Cary','Somerset','England'),
    ('Chard','Somerset','England'),
    ('Clevedon','Somerset','England'),
    ('Crewkerne','Somerset','England'),
    ('Dulverton','Somerset','England'),
    ('Frome','Somerset','England'),
    ('Glastonbury','Somerset','England'),
    ('Highbridge','Somerset','England'),
    ('Ilminster','Somerset','England'),
    ('Keynsham','Somerset','England'),
    ('Langport','Somerset','England'),
    ('Midsomer Norton','Somerset','England'),
    ('Minehead','Somerset','England'),
    ('Nailsea','Somerset','England'),
    ('North Petherton','Somerset','England'),
    ('Norton Radstock','Somerset','England'),
    ('Portishead','Somerset','England'),
    ('Portishead and North Weston','Somerset','England'),
    ('Radstock','Somerset','England'),
    ('Shepton Mallet','Somerset','England'),
    ('Somerton','Somerset','England'),
    ('South Petherton','Somerset','England'),
    ('Street','Somerset','England'),
    ('Taunton','Somerset','England'),
    ('Watchet','Somerset','England'),
    ('Wellington','Somerset','England'),
    ('Wells','Somerset','England'),
    ('Weston super Mare','Somerset','England'),
    ('Wincanton','Somerset','England'),
    ('Winsford','Somerset','England'),
    ('Wiveliscombe','Somerset','England'),
    ('Yeovil','Somerset','England'),
    ('Anston','South Yorkshire','England'),
    ('Askern','South Yorkshire','England'),
    ('Barnsley','South Yorkshire','England'),
    ('Bawtry','South Yorkshire','England'),
    ('Brierley','South Yorkshire','England'),
    ('Conisbrough','South Yorkshire','England'),
    ('Dinnington','South Yorkshire','England'),
    ('Doncaster','South Yorkshire','England'),
    ('Edlington','South Yorkshire','England'),
    ('Hatfield','South Yorkshire','England'),
    ('Hoyland','South Yorkshire','England'),
    ('Maltby','South Yorkshire','England'),
    ('Mexborough','South Yorkshire','England'),
    ('Penistone','South Yorkshire','England'),
    ('Rotherham','South Yorkshire','England'),
    ('Sheffield','South Yorkshire','England'),
    ('Stainforth','South Yorkshire','England');
INSERT INTO towns VALUES
    ('Stocksbridge','South Yorkshire','England'),
    ('Swinton','South Yorkshire','England'),
    ('Thorne','South Yorkshire','England'),
    ('Tickhill','South Yorkshire','England'),
    ('Wath upon Dearne','South Yorkshire','England'),
    ('Wombwell','South Yorkshire','England'),
    ('Alton','Staffordshire','England'),
    ('Biddulph','Staffordshire','England'),
    ('Burntwood','Staffordshire','England'),
    ('Burslem','Staffordshire','England'),
    ('Burton upon Trent','Staffordshire','England'),
    ('Cannock','Staffordshire','England'),
    ('Cheadle','Staffordshire','England'),
    ('Eccleshall','Staffordshire','England'),
    ('Fazeley','Staffordshire','England'),
    ('Fenton','Staffordshire','England'),
    ('Hednesford','Staffordshire','England'),
    ('Kidsgrove','Staffordshire','England'),
    ('Leek','Staffordshire','England'),
    ('Lichfield','Staffordshire','England'),
    ('Longton','Staffordshire','England'),
    ('Newcastle under Lyme','Staffordshire','England'),
    ('Penkridge','Staffordshire','England'),
    ('Perry Crofts','Staffordshire','England'),
    ('Rugeley','Staffordshire','England'),
    ('Stafford','Staffordshire','England'),
    ('Stoke on Trent','Staffordshire','England'),
    ('Stone','Staffordshire','England'),
    ('Tamworth','Staffordshire','England'),
    ('Tunstall','Staffordshire','England'),
    ('Uttoxeter','Staffordshire','England'),
    ('Aldeburgh','Suffolk','England'),
    ('Beccles','Suffolk','England'),
    ('Brandon','Suffolk','England'),
    ('Bungay','Suffolk','England'),
    ('Bury St Edmunds','Suffolk','England'),
    ('Carlton Colville','Suffolk','England'),
    ('Clare','Suffolk','England'),
    ('Dommoc','Suffolk','England'),
    ('Dunwich','Suffolk','England'),
    ('Eye','Suffolk','England'),
    ('Felixstowe','Suffolk','England'),
    ('Framlingham','Suffolk','England'),
    ('Hadleigh','Suffolk','England'),
    ('Halesworth','Suffolk','England'),
    ('Haverhill','Suffolk','England'),
    ('Ipswich','Suffolk','England'),
    ('Kesgrave','Suffolk','England'),
    ('Leiston','Suffolk','England'),
    ('Lowestoft','Suffolk','England'),
    ('Mildenhall','Suffolk','England'),
    ('Needham Market','Suffolk','England'),
    ('Newmarket','Suffolk','England'),
    ('Orford','Suffolk','England'),
    ('Otley','Suffolk','England'),
    ('Saxmundham','Suffolk','England'),
    ('Southwold','Suffolk','England'),
    ('Stowmarket','Suffolk','England'),
    ('Sudbury','Suffolk','England'),
    ('Woodbridge','Suffolk','England'),
    ('Ashford','Surrey','England'),
    ('Beltchingley','Surrey','England'),
    ('Camberley','Surrey','England'),
    ('Chertsey','Surrey','England'),
    ('Croydon','Surrey','England'),
    ('Dorking','Surrey','England'),
    ('Egham','Surrey','England'),
    ('Epsom','Surrey','England'),
    ('Esher','Surrey','England'),
    ('Farnham','Surrey','England'),
    ('Godalming','Surrey','England'),
    ('Gomshall','Surrey','England'),
    ('Gratton','Surrey','England'),
    ('Great Brookham','Surrey','England'),
    ('Guildford','Surrey','England'),
    ('Haslemere','Surrey','England'),
    ('Horley','Surrey','England'),
    ('Kingston upon Thames','Surrey','England'),
    ('Leatherhead','Surrey','England'),
    ('Redhill','Surrey','England'),
    ('Reigate','Surrey','England'),
    ('Southwark','Surrey','England'),
    ('Staines upon Thames','Surrey','England'),
    ('Walton on Thames','Surrey','England'),
    ('Weybridge','Surrey','England'),
    ('Woking','Surrey','England'),
    ('Birtley','Tyne and Wear','England'),
    ('Blaydon on tyne','Tyne and Wear','England'),
    ('Cullercoats','Tyne and Wear','England'),
    ('Darsley Park','Tyne and Wear','England'),
    ('Dunston','Tyne and Wear','England'),
    ('Gateshead','Tyne and Wear','England'),
    ('Hetton','Tyne and Wear','England'),
    ('Houghton le Spring','Tyne and Wear','England'),
    ('Howdon','Tyne and Wear','England'),
    ('Jarrow','Tyne and Wear','England'),
    ('Killingworth','Tyne and Wear','England'),
    ('Little Benton','Tyne and Wear','England'),
    ('Longbenton','Tyne and Wear','England'),
    ('Low Fell','Tyne and Wear','England'),
    ('Newcastle upon Tyne','Tyne and Wear','England'),
    ('North Shields','Tyne and Wear','England'),
    ('Ryton','Tyne and Wear','England'),
    ('Sheriff Hill','Tyne and Wear','England'),
    ('South Shields','Tyne and Wear','England'),
    ('Sunderland','Tyne and Wear','England'),
    ('Tynemouth','Tyne and Wear','England'),
    ('Wallsend','Tyne and Wear','England'),
    ('Washington','Tyne and Wear','England'),
    ('Whitley Bay','Tyne and Wear','England'),
    ('Willington Quay','Tyne and Wear','England'),
    ('Windy Nook','Tyne and Wear','England'),
    ('Alcester','Warwickshire','England'),
    ('Atherstone','Warwickshire','England'),
    ('Bedworth','Warwickshire','England'),
    ('Coleshill','Warwickshire','England'),
    ('Henley in Arden','Warwickshire','England'),
    ('Kenilworth','Warwickshire','England'),
    ('Middle Quinton','Warwickshire','England'),
    ('Nuneaton','Warwickshire','England'),
    ('Royal Leamington Spa','Warwickshire','England'),
    ('Rugby','Warwickshire','England'),
    ('Shipston on Stour','Warwickshire','England'),
    ('Southam','Warwickshire','England'),
    ('Stratford upon Avon','Warwickshire','England'),
    ('Warwick','Warwickshire','England'),
    ('Whitnash','Warwickshire','England'),
    ('Aldridge','West Midlands','England'),
    ('Bilston','West Midlands','England'),
    ('Birmingham','West Midlands','England'),
    ('Blackheath','West Midlands','England'),
    ('Bloxwich','West Midlands','England'),
    ('Brierley Hill','West Midlands','England'),
    ('Brownhills','West Midlands','England'),
    ('Coal Pool','West Midlands','England'),
    ('Coseley','West Midlands','England'),
    ('Coventry','West Midlands','England'),
    ('Cradley Heath','West Midlands','England'),
    ('Darlaston','West Midlands','England'),
    ('Dudley','West Midlands','England'),
    ('Fordbridge','West Midlands','England'),
    ('Gornal','West Midlands','England'),
    ('Halesowen','West Midlands','England'),
    ('Lye','West Midlands','England'),
    ('Moxley','West Midlands','England'),
    ('Netherton','West Midlands','England'),
    ('Oldbury','West Midlands','England'),
    ('Pelsall','West Midlands','England'),
    ('Rowley Regis','West Midlands','England'),
    ('Sedgley','West Midlands','England'),
    ('Smethwick','West Midlands','England'),
    ('Solihull','West Midlands','England'),
    ('Stourbridge','West Midlands','England'),
    ('Sutton Coldfield','West Midlands','England'),
    ('Tipton','West Midlands','England'),
    ('Walsall','West Midlands','England'),
    ('Wednesbury','West Midlands','England'),
    ('Wednesfield','West Midlands','England'),
    ('West Bromwich','West Midlands','England'),
    ('Willenhall','West Midlands','England'),
    ('Wolverhampton','West Midlands','England'),
    ('Arundel','West Sussex','England'),
    ('Bognor Regis','West Sussex','England'),
    ('Burgess Hill','West Sussex','England'),
    ('Chichester','West Sussex','England'),
    ('Crawley','West Sussex','England'),
    ('Cuckfield','West Sussex','England'),
    ('East Grinstead','West Sussex','England'),
    ('Haywards Heath','West Sussex','England'),
    ('Horsham','West Sussex','England'),
    ('Littlehampton','West Sussex','England'),
    ('Midhurst','West Sussex','England'),
    ('Petworth','West Sussex','England'),
    ('Selsey','West Sussex','England'),
    ('Shoreham by Sea','West Sussex','England'),
    ('Southwick','West Sussex','England'),
    ('Steyning','West Sussex','England'),
    ('Worthing','West Sussex','England'),
    ('Baildon','West Yorkshire','England'),
    ('Batley','West Yorkshire','England'),
    ('Bingley','West Yorkshire','England'),
    ('Bradford','West Yorkshire','England'),
    ('Brighouse','West Yorkshire','England'),
    ('Castleford','West Yorkshire','England'),
    ('Cleckheaton','West Yorkshire','England'),
    ('Denholme','West Yorkshire','England'),
    ('Dewsbury','West Yorkshire','England'),
    ('Elland','West Yorkshire','England'),
    ('Farsley','West Yorkshire','England'),
    ('Featherstone','West Yorkshire','England'),
    ('Garforth','West Yorkshire','England'),
    ('Guiseley','West Yorkshire','England'),
    ('Halifax','West Yorkshire','England'),
    ('Hebden Bridge','West Yorkshire','England'),
    ('Hebden Royd','West Yorkshire','England'),
    ('Heckmondwike','West Yorkshire','England'),
    ('Hemsworth','West Yorkshire','England'),
    ('Holmfirth','West Yorkshire','England'),
    ('Horsforth','West Yorkshire','England'),
    ('Huddersfield','West Yorkshire','England'),
    ('Ilkley','West Yorkshire','England'),
    ('Keighley','West Yorkshire','England'),
    ('Knottingley','West Yorkshire','England'),
    ('Leeds','West Yorkshire','England'),
    ('Meltham','West Yorkshire','England'),
    ('Mirfield','West Yorkshire','England'),
    ('Morley','West Yorkshire','England'),
    ('Mytholmroyd','West Yorkshire','England'),
    ('Normanton','West Yorkshire','England'),
    ('Ossett','West Yorkshire','England'),
    ('Otley','West Yorkshire','England'),
    ('Pontefract','West Yorkshire','England'),
    ('Pudsey','West Yorkshire','England'),
    ('Rothwell','West Yorkshire','England'),
    ('Shipley','West Yorkshire','England'),
    ('Silsden','West Yorkshire','England'),
    ('South Elmsall','West Yorkshire','England'),
    ('South Kirkby and Moorthorpe','West Yorkshire','England'),
    ('Sowerby Bridge','West Yorkshire','England'),
    ('Todmorden','West Yorkshire','England'),
    ('Wakefield','West Yorkshire','England'),
    ('Wetherby','West Yorkshire','England'),
    ('Yeadon','West Yorkshire','England'),
    ('Amesbury','Wiltshire','England'),
    ('Bradford on Avon','Wiltshire','England'),
    ('Calne','Wiltshire','England'),
    ('Chippenham','Wiltshire','England'),
    ('Corsham','Wiltshire','England'),
    ('Cricklade','Wiltshire','England'),
    ('Devizes','Wiltshire','England'),
    ('Highworth','Wiltshire','England'),
    ('Ludgershall','Wiltshire','England'),
    ('Malmesbury','Wiltshire','England'),
    ('Marlborough','Wiltshire','England'),
    ('Melksham','Wiltshire','England'),
    ('Mere','Wiltshire','England'),
    ('Royal Wootton Bassett','Wiltshire','England'),
    ('Salisbury','Wiltshire','England'),
    ('Swindon','Wiltshire','England'),
    ('Tidworth','Wiltshire','England'),
    ('Tisbury','Wiltshire','England'),
    ('Trowbridge','Wiltshire','England'),
    ('Warminster','Wiltshire','England'),
    ('Westbury','Wiltshire','England'),
    ('Wilton','Wiltshire','England'),
    ('Bewdley','Worcestershire','England'),
    ('Bromsgrove','Worcestershire','England'),
    ('Droitwich Spa','Worcestershire','England'),
    ('Evesham','Worcestershire','England'),
    ('Great Malvern','Worcestershire','England');
INSERT INTO towns VALUES
    ('Kidderminster','Worcestershire','England'),
    ('Malvern','Worcestershire','England'),
    ('Pershore','Worcestershire','England'),
    ('Redditch','Worcestershire','England'),
    ('Stourport on Severn','Worcestershire','England'),
    ('Tenbury Wells','Worcestershire','England'),
    ('Upton upon Severn','Worcestershire','England'),
    ('Worcester','Worcestershire','England'),
    ('Antrim','County Antrim','Northern Ireland'),
    ('Ballycastle','County Antrim','Northern Ireland'),
    ('Ballyclare','County Antrim','Northern Ireland'),
    ('Ballymena','County Antrim','Northern Ireland'),
    ('Ballymoney','County Antrim','Northern Ireland'),
    ('Bushmills','County Antrim','Northern Ireland'),
    ('Carrickfergus','County Antrim','Northern Ireland'),
    ('Crumlin','County Antrim','Northern Ireland'),
    ('Greenisland','County Antrim','Northern Ireland'),
    ('Larne','County Antrim','Northern Ireland'),
    ('Lisburn','County Antrim','Northern Ireland'),
    ('Merville Garden Village','County Antrim','Northern Ireland'),
    ('Newtownabbey','County Antrim','Northern Ireland'),
    ('Portrush','County Antrim','Northern Ireland'),
    ('Randalstown','County Antrim','Northern Ireland'),
    ('Armagh','County Armagh','Northern Ireland'),
    ('Craigavon','County Armagh','Northern Ireland'),
    ('Lurgan','County Armagh','Northern Ireland'),
    ('Markethill','County Armagh','Northern Ireland'),
    ('Newry','County Armagh','Northern Ireland'),
    ('Portadown','County Armagh','Northern Ireland'),
    ('Ballynahinch','County Down','Northern Ireland'),
    ('Banbridge','County Down','Northern Ireland'),
    ('Bangor','County Down','Northern Ireland'),
    ('Carryduff','County Down','Northern Ireland'),
    ('Castlewellan','County Down','Northern Ireland'),
    ('Comber','County Down','Northern Ireland'),
    ('Donaghadee','County Down','Northern Ireland'),
    ('Doromore','County Down','Northern Ireland'),
    ('Downpatrick','County Down','Northern Ireland'),
    ('Dundonald','County Down','Northern Ireland'),
    ('Holywood','County Down','Northern Ireland'),
    ('Kilkeel','County Down','Northern Ireland'),
    ('Killyleagh','County Down','Northern Ireland'),
    ('Lisburn','County Down','Northern Ireland'),
    ('Newcastle','County Down','Northern Ireland'),
    ('Newtownards','County Down','Northern Ireland'),
    ('Portaferry','County Down','Northern Ireland'),
    ('Rostrevor','County Down','Northern Ireland'),
    ('Saintfield','County Down','Northern Ireland'),
    ('Warrenpoint','County Down','Northern Ireland'),
    ('Enniskillen','County Fermanagh','Northern Ireland'),
    ('Lisnaskea','County Fermanagh','Northern Ireland'),
    ('Coleraine','County Londonderry','Northern Ireland'),
    ('Derry','County Londonderry','Northern Ireland'),
    ('Limavady','County Londonderry','Northern Ireland'),
    ('Magherafelt','County Londonderry','Northern Ireland'),
    ('Portstewart','County Londonderry','Northern Ireland'),
    ('Castlederg','County Tyrone','Northern Ireland'),
    ('Clogher','County Tyrone','Northern Ireland'),
    ('Coalisland','County Tyrone','Northern Ireland'),
    ('Cookstown','County Tyrone','Northern Ireland'),
    ('Dungannon','County Tyrone','Northern Ireland'),
    ('Fintona','County Tyrone','Northern Ireland'),
    ('Fivemiletown','County Tyrone','Northern Ireland'),
    ('Omagh','County Tyrone','Northern Ireland'),
    ('Strabane','County Tyrone','Northern Ireland'),
    ('Aberdeen','Aberdeenshire','Scotland'),
    ('Alford','Aberdeenshire','Scotland'),
    ('Ballater','Aberdeenshire','Scotland'),
    ('Banchory','Aberdeenshire','Scotland'),
    ('Banff','Aberdeenshire','Scotland'),
    ('Banff and Macduff','Aberdeenshire','Scotland'),
    ('Blackburn','Aberdeenshire','Scotland'),
    ('Boddam','Aberdeenshire','Scotland'),
    ('Clerkhill','Aberdeenshire','Scotland'),
    ('Ellon','Aberdeenshire','Scotland'),
    ('Fraserburgh','Aberdeenshire','Scotland'),
    ('Huntly','Aberdeenshire','Scotland'),
    ('Insch','Aberdeenshire','Scotland'),
    ('Inverbervie','Aberdeenshire','Scotland'),
    ('Inverurie','Aberdeenshire','Scotland'),
    ('Kemnay','Aberdeenshire','Scotland'),
    ('Kintore','Aberdeenshire','Scotland'),
    ('Laurencekirk','Aberdeenshire','Scotland'),
    ('Macduff','Aberdeenshire','Scotland'),
    ('Maud','Aberdeenshire','Scotland'),
    ('Oldmeldrum','Aberdeenshire','Scotland'),
    ('Peterhead','Aberdeenshire','Scotland'),
    ('Portlethen','Aberdeenshire','Scotland'),
    ('Portsoy','Aberdeenshire','Scotland'),
    ('Red Cloak','Aberdeenshire','Scotland'),
    ('Rosehearty','Aberdeenshire','Scotland'),
    ('Stonehaven','Aberdeenshire','Scotland'),
    ('Turriff','Aberdeenshire','Scotland'),
    ('Westhill','Aberdeenshire','Scotland'),
    ('Arbroath','Angus','Scotland'),
    ('Brechin','Angus','Scotland'),
    ('Carnoustie','Angus','Scotland'),
    ('Forfar','Angus','Scotland'),
    ('Kirriemuir','Angus','Scotland'),
    ('Monifieth','Angus','Scotland'),
    ('Montrose','Angus','Scotland'),
    ('Alloa','Clackmannanshire','Scotland'),
    ('Alva','Clackmannanshire','Scotland'),
    ('Clackmannan','Clackmannanshire','Scotland'),
    ('Dollar','Clackmannanshire','Scotland'),
    ('Menstrie','Clackmannanshire','Scotland'),
    ('Tillicoultry','Clackmannanshire','Scotland'),
    ('Tullibody','Clackmannanshire','Scotland'),
    ('Annan','Dumfries and Galloway','Scotland'),
    ('Castle Douglas','Dumfries and Galloway','Scotland'),
    ('Dalbeattie','Dumfries and Galloway','Scotland'),
    ('Dumfries','Dumfries and Galloway','Scotland'),
    ('Gatehouse of Fleet','Dumfries and Galloway','Scotland'),
    ('Gretna','Dumfries and Galloway','Scotland'),
    ('Kelloholm','Dumfries and Galloway','Scotland'),
    ('Kirkconnel','Dumfries and Galloway','Scotland'),
    ('Kirkcudbright','Dumfries and Galloway','Scotland'),
    ('Langholm','Dumfries and Galloway','Scotland'),
    ('Lochmaben','Dumfries and Galloway','Scotland'),
    ('Lockerbie','Dumfries and Galloway','Scotland'),
    ('Moffat','Dumfries and Galloway','Scotland'),
    ('Monreith','Dumfries and Galloway','Scotland'),
    ('Newbridge Drive','Dumfries and Galloway','Scotland'),
    ('Newton Stewart','Dumfries and Galloway','Scotland'),
    ('Sanquhar','Dumfries and Galloway','Scotland'),
    ('Stranraer','Dumfries and Galloway','Scotland'),
    ('Thornhill','Dumfries and Galloway','Scotland'),
    ('Wigtown','Dumfries and Galloway','Scotland'),
    ('Dundee','Dundee','Scotland'),
    ('Cockenzie and Port Seton','East Lothian','Scotland'),
    ('Dunbar','East Lothian','Scotland'),
    ('Haddington','East Lothian','Scotland'),
    ('Musselburgh','East Lothian','Scotland'),
    ('North Berwick','East Lothian','Scotland'),
    ('Prestonpans','East Lothian','Scotland'),
    ('Tranent','East Lothian','Scotland'),
    ('Edinburgh?','Edinburgh?','Scotland'),
    ('Airth','Falkirk','Scotland'),
    ('Bo''ness','Falkirk','Scotland'),
    ('Bonnybridge','Falkirk','Scotland'),
    ('Borrowstounness','Falkirk','Scotland'),
    ('Carron','Falkirk','Scotland'),
    ('Denny','Falkirk','Scotland'),
    ('Duniplace','Falkirk','Scotland'),
    ('Dunmore','Falkirk','Scotland'),
    ('Falkirk','Falkirk','Scotland'),
    ('Grahamston','Falkirk','Scotland'),
    ('Grangemouth','Falkirk','Scotland'),
    ('Larbert','Falkirk','Scotland'),
    ('Stenhousemuir','Falkirk','Scotland'),
    ('Anstruther','Fife','Scotland'),
    ('Auchtermuchty','Fife','Scotland'),
    ('Balcurvie','Fife','Scotland'),
    ('Ballingry','Fife','Scotland'),
    ('Benarty','Fife','Scotland'),
    ('Buckhaven','Fife','Scotland'),
    ('Burntisland','Fife','Scotland'),
    ('Collydean','Fife','Scotland'),
    ('Cowdenbeath','Fife','Scotland'),
    ('Crail','Fife','Scotland'),
    ('Cupar','Fife','Scotland'),
    ('Dalgety Bay','Fife','Scotland'),
    ('Dunfermline','Fife','Scotland'),
    ('Dysart','Fife','Scotland'),
    ('East Wemyss','Fife','Scotland'),
    ('Falkland','Fife','Scotland'),
    ('Glenrothes','Fife','Scotland'),
    ('Inverkeithing','Fife','Scotland'),
    ('Kelty','Fife','Scotland'),
    ('Kincardine','Fife','Scotland'),
    ('Kinghorn','Fife','Scotland'),
    ('Kinglassie','Fife','Scotland'),
    ('Kirkcaldy','Fife','Scotland'),
    ('Ladybank','Fife','Scotland'),
    ('Letham','Fife','Scotland'),
    ('Leuchars','Fife','Scotland'),
    ('Leven','Fife','Scotland'),
    ('Levenmouth','Fife','Scotland'),
    ('Lochgelly','Fife','Scotland'),
    ('Markinch','Fife','Scotland'),
    ('Methil','Fife','Scotland'),
    ('Newburgh','Fife','Scotland'),
    ('Newport on Tay','Fife','Scotland'),
    ('North Queensferry','Fife','Scotland'),
    ('Pitcoudie','Fife','Scotland'),
    ('Pittenweem','Fife','Scotland'),
    ('Rosyth','Fife','Scotland'),
    ('St Andrews','Fife','Scotland'),
    ('St Monans','Fife','Scotland'),
    ('Tayport','Fife','Scotland'),
    ('Woodhaven','Fife','Scotland'),
    ('Wormit','Fife','Scotland'),
    ('Alness','Highlands','Scotland'),
    ('Aviemore','Highlands','Scotland'),
    ('Brora','Highlands','Scotland'),
    ('Cromarty','Highlands','Scotland'),
    ('Dingwall','Highlands','Scotland'),
    ('Dornoch','Highlands','Scotland'),
    ('Fort William','Highlands','Scotland'),
    ('Fortrose','Highlands','Scotland'),
    ('Grantown on Spey','Highlands','Scotland'),
    ('Invergordon','Highlands','Scotland'),
    ('Inverlochy','Highlands','Scotland'),
    ('Inverness','Highlands','Scotland'),
    ('Kingussie','Highlands','Scotland'),
    ('Mallaig','Highlands','Scotland'),
    ('Nairn','Highlands','Scotland'),
    ('Portree','Highlands','Scotland'),
    ('Tain','Highlands','Scotland'),
    ('Thurso','Highlands','Scotland'),
    ('Ullapool','Highlands','Scotland'),
    ('Wick','Highlands','Scotland'),
    ('Bathgate','Lothian','Scotland'),
    ('Dalkeith','Lothian','Scotland'),
    ('Dunbar','Lothian','Scotland'),
    ('Edinburgh','Lothian','Scotland'),
    ('Haddington','Lothian','Scotland'),
    ('Linlithgow','Lothian','Scotland'),
    ('Loanhead','Lothian','Scotland'),
    ('Musselburgh','Lothian','Scotland'),
    ('North Berwick','Lothian','Scotland'),
    ('Penicuik','Lothian','Scotland'),
    ('Buckie','Moray','Scotland'),
    ('Cullen','Moray','Scotland'),
    ('Dufftown','Moray','Scotland'),
    ('Elgin','Moray','Scotland'),
    ('Forres','Moray','Scotland'),
    ('Keith','Moray','Scotland'),
    ('Lossiemouth','Moray','Scotland'),
    ('Aberfeldy','Perth and Kinross','Scotland'),
    ('Auchterarder','Perth and Kinross','Scotland'),
    ('Birnam','Perth and Kinross','Scotland'),
    ('Blackford','Perth and Kinross','Scotland'),
    ('Blair Atholl','Perth and Kinross','Scotland'),
    ('Blairgowrie and Rattray','Perth and Kinross','Scotland'),
    ('Bridge of Earn','Perth and Kinross','Scotland'),
    ('Bridge of Tilt','Perth and Kinross','Scotland'),
    ('Comrie','Perth and Kinross','Scotland'),
    ('Coupar Angus','Perth and Kinross','Scotland'),
    ('Crieff','Perth and Kinross','Scotland'),
    ('Kinross','Perth and Kinross','Scotland'),
    ('Perth','Perth and Kinross','Scotland'),
    ('Pitlochry','Perth and Kinross','Scotland'),
    ('Scone','Perth and Kinross','Scotland'),
    ('Coldstream','Scottish Borders','Scotland'),
    ('Duns','Scottish Borders','Scotland'),
    ('Earlston','Scottish Borders','Scotland'),
    ('Eyemouth','Scottish Borders','Scotland'),
    ('Galashiels','Scottish Borders','Scotland'),
    ('Greenlaw','Scottish Borders','Scotland');
INSERT INTO towns VALUES
    ('Hawick','Scottish Borders','Scotland'),
    ('Innerleithen','Scottish Borders','Scotland'),
    ('Jedburgh','Scottish Borders','Scotland'),
    ('Kelso','Scottish Borders','Scotland'),
    ('Lauder','Scottish Borders','Scotland'),
    ('Melrose','Scottish Borders','Scotland'),
    ('Newtown St Boswells','Scottish Borders','Scotland'),
    ('Peebles','Scottish Borders','Scotland'),
    ('Selkirk','Scottish Borders','Scotland'),
    ('Bridge of Allan','Stirlingshire','Scotland'),
    ('Callander','Stirlingshire','Scotland'),
    ('Doune','Stirlingshire','Scotland'),
    ('Dunblane','Stirlingshire','Scotland'),
    ('Stirling','Stirlingshire','Scotland'),
    ('Airdrie','Strathclyde','Scotland'),
    ('Ayr','Strathclyde','Scotland'),
    ('Barrhead','Strathclyde','Scotland'),
    ('Bearsden','Strathclyde','Scotland'),
    ('Bellshill','Strathclyde','Scotland'),
    ('Biggar','Strathclyde','Scotland'),
    ('Campbeltown','Strathclyde','Scotland'),
    ('Carluke','Strathclyde','Scotland'),
    ('Clydebank','Strathclyde','Scotland'),
    ('Coatbridge','Strathclyde','Scotland'),
    ('Cumbernauld','Strathclyde','Scotland'),
    ('Dumbarton','Strathclyde','Scotland'),
    ('Dunoon','Strathclyde','Scotland'),
    ('East Kilbride','Strathclyde','Scotland'),
    ('Glasgow','Strathclyde','Scotland'),
    ('Gourock','Strathclyde','Scotland'),
    ('Greenock','Strathclyde','Scotland'),
    ('Hamilton','Strathclyde','Scotland'),
    ('Helensburgh','Strathclyde','Scotland'),
    ('Inveraray','Strathclyde','Scotland'),
    ('Irvine','Strathclyde','Scotland'),
    ('Johnstone','Strathclyde','Scotland'),
    ('Kilbarchan','Strathclyde','Scotland'),
    ('Kilmarnock','Strathclyde','Scotland'),
    ('Kilwinning','Strathclyde','Scotland'),
    ('Lanark','Strathclyde','Scotland'),
    ('Largs','Strathclyde','Scotland'),
    ('Lochgilphead','Strathclyde','Scotland'),
    ('Maybole','Strathclyde','Scotland'),
    ('Milngavie','Strathclyde','Scotland'),
    ('Motherwell','Strathclyde','Scotland'),
    ('Oban','Strathclyde','Scotland'),
    ('Paisley','Strathclyde','Scotland'),
    ('Prestwick','Strathclyde','Scotland'),
    ('Rothesay','Strathclyde','Scotland'),
    ('Rutherglen','Strathclyde','Scotland'),
    ('Saltcoats','Strathclyde','Scotland'),
    ('Tobermory','Strathclyde','Scotland'),
    ('Troon','Strathclyde','Scotland'),
    ('Armadale','West Lothian','Scotland'),
    ('Ballencrieff','West Lothian','Scotland'),
    ('Bathgate','West Lothian','Scotland'),
    ('Blackburn','West Lothian','Scotland'),
    ('Blackridge','West Lothian','Scotland'),
    ('Bridgend','West Lothian','Scotland'),
    ('Broxburn','West Lothian','Scotland'),
    ('East Calder','West Lothian','Scotland'),
    ('Linlithgow','West Lothian','Scotland'),
    ('Livingston','West Lothian','Scotland'),
    ('Stoneyburn','West Lothian','Scotland'),
    ('Whitburn','West Lothian','Scotland'),
    ('Kirkwall','Western Isles','Scotland'),
    ('Lerwick','Western Isles','Scotland'),
    ('Stornoway','Western Isles','Scotland'),
    ('Amlwch','Anglesey','Wales'),
    ('Beaumaris','Anglesey','Wales'),
    ('Benllech','Anglesey','Wales'),
    ('Holyhead','Anglesey','Wales'),
    ('Llanfairpwllgwyngyll','Anglesey','Wales'),
    ('Llangefni','Anglesey','Wales'),
    ('Menai Bridge','Anglesey','Wales'),
    ('Ammanford','Carmarthenshire','Wales'),
    ('Burry Port','Carmarthenshire','Wales'),
    ('Carmarthen','Carmarthenshire','Wales'),
    ('Garnant','Carmarthenshire','Wales'),
    ('Glanamman','Carmarthenshire','Wales'),
    ('Kidwelly','Carmarthenshire','Wales'),
    ('Laugharne','Carmarthenshire','Wales'),
    ('Llandeilo','Carmarthenshire','Wales'),
    ('Llandovery','Carmarthenshire','Wales'),
    ('Llanelli','Carmarthenshire','Wales'),
    ('Newcastle Emlyn','Carmarthenshire','Wales'),
    ('St Clears','Carmarthenshire','Wales'),
    ('Whitland','Carmarthenshire','Wales'),
    ('Aberaeron','Ceredigion','Wales'),
    ('Aberystwyth','Ceredigion','Wales'),
    ('Cardigan','Ceredigion','Wales'),
    ('Lampeter','Ceredigion','Wales'),
    ('Llandysul','Ceredigion','Wales'),
    ('New Quay','Ceredigion','Wales'),
    ('Tregaron','Ceredigion','Wales'),
    ('Abergele','Conwy','Wales'),
    ('Colwyn Bay','Conwy','Wales'),
    ('Conwy','Conwy','Wales'),
    ('Deganwy','Conwy','Wales'),
    ('Llandudno','Conwy','Wales'),
    ('Llandudno Junction','Conwy','Wales'),
    ('Llanfairfechan','Conwy','Wales'),
    ('Llanrwst','Conwy','Wales'),
    ('Old Colwyn','Conwy','Wales'),
    ('Penmaenmawr','Conwy','Wales'),
    ('Towyn','Conwy','Wales'),
    ('Carrog','Denbighshire','Wales'),
    ('Corwen','Denbighshire','Wales'),
    ('Denbigh','Denbighshire','Wales'),
    ('Gellifor','Denbighshire','Wales'),
    ('Llangollen','Denbighshire','Wales'),
    ('Prestatyn','Denbighshire','Wales'),
    ('Rhuddlan','Denbighshire','Wales'),
    ('Rhyl','Denbighshire','Wales'),
    ('Ruthin','Denbighshire','Wales'),
    ('St Asaph','Denbighshire','Wales'),
    ('Bagallit','Flintshire','Wales'),
    ('Broughton','Flintshire','Wales'),
    ('Buckley','Flintshire','Wales'),
    ('Caerwys','Flintshire','Wales'),
    ('Connah''s Quay','Flintshire','Wales'),
    ('Ewole','Flintshire','Wales'),
    ('Flint','Flintshire','Wales'),
    ('Hawarden','Flintshire','Wales'),
    ('Holywell','Flintshire','Wales'),
    ('Mold','Flintshire','Wales'),
    ('Queensferry','Flintshire','Wales'),
    ('Saltney','Flintshire','Wales'),
    ('Shotton','Flintshire','Wales'),
    ('Abertillery','Gwent','Wales'),
    ('Blaina','Gwent','Wales'),
    ('Brynmawr','Gwent','Wales'),
    ('Cwmbran','Gwent','Wales'),
    ('Ebbw Vale','Gwent','Wales'),
    ('Newport','Gwent','Wales'),
    ('Tafarnaubach','Gwent','Wales'),
    ('Tredegar','Gwent','Wales'),
    ('Bala','Gwynedd','Wales'),
    ('Bangor','Gwynedd','Wales'),
    ('Barmouth','Gwynedd','Wales'),
    ('Beaumaris','Gwynedd','Wales'),
    ('Betws y Coed','Gwynedd','Wales'),
    ('Blaenau Ffestiniog','Gwynedd','Wales'),
    ('Caernarfon','Gwynedd','Wales'),
    ('Conwy','Gwynedd','Wales'),
    ('Criccieth','Gwynedd','Wales'),
    ('Dolgellau','Gwynedd','Wales'),
    ('Ffestiniog','Gwynedd','Wales'),
    ('Harlech','Gwynedd','Wales'),
    ('Holyhead','Gwynedd','Wales'),
    ('Llanberis','Gwynedd','Wales'),
    ('Llanfachreth','Gwynedd','Wales'),
    ('Nefyn','Gwynedd','Wales'),
    ('Porthmadog','Gwynedd','Wales'),
    ('Pwllheli','Gwynedd','Wales'),
    ('Tywyn','Gwynedd','Wales'),
    ('Y Felinheli','Gwynedd','Wales'),
    ('Aberdare','Mid Glamorgan','Wales'),
    ('Bridgend','Mid Glamorgan','Wales'),
    ('Caerphilly','Mid Glamorgan','Wales'),
    ('Llantrisant','Mid Glamorgan','Wales'),
    ('Maesteg','Mid Glamorgan','Wales'),
    ('Merthyr Tydfil','Mid Glamorgan','Wales'),
    ('Pontypridd','Mid Glamorgan','Wales'),
    ('Porth','Mid Glamorgan','Wales'),
    ('Porthcawl','Mid Glamorgan','Wales'),
    ('Abergavenny','Monmouthshire','Wales'),
    ('Caldicot','Monmouthshire','Wales'),
    ('Chepstow','Monmouthshire','Wales'),
    ('Monmouth','Monmouthshire','Wales'),
    ('Usk','Monmouthshire','Wales'),
    ('Fishguard','Pembrokeshire','Wales'),
    ('Goodwick','Pembrokeshire','Wales'),
    ('Hakin','Pembrokeshire','Wales'),
    ('Haverfordwest','Pembrokeshire','Wales'),
    ('Milford Haven','Pembrokeshire','Wales'),
    ('Narberth','Pembrokeshire','Wales'),
    ('Newport','Pembrokeshire','Wales'),
    ('Neyland','Pembrokeshire','Wales'),
    ('Pembroke','Pembrokeshire','Wales'),
    ('Pembroke Dock','Pembrokeshire','Wales'),
    ('St Davids','Pembrokeshire','Wales'),
    ('Tenby','Pembrokeshire','Wales'),
    ('Brecon','Powys','Wales'),
    ('Builth Wells','Powys','Wales'),
    ('Cefnllys','Powys','Wales'),
    ('Crickhowell','Powys','Wales'),
    ('Hay on Wye','Powys','Wales'),
    ('Knighton','Powys','Wales'),
    ('Llandrindod Wells','Powys','Wales'),
    ('Llanfair Caereinion','Powys','Wales'),
    ('Llanfyllin','Powys','Wales'),
    ('Llangors','Powys','Wales'),
    ('Llanidloes','Powys','Wales'),
    ('Llanwrtyd Wells','Powys','Wales'),
    ('Machynlleth','Powys','Wales'),
    ('Montgomery','Powys','Wales'),
    ('Newtown','Powys','Wales'),
    ('Old Radnor','Powys','Wales'),
    ('Presteigne','Powys','Wales'),
    ('Rhayader','Powys','Wales'),
    ('Talgarth','Powys','Wales'),
    ('Welshpool','Powys','Wales'),
    ('Barry','South Glamorgan','Wales'),
    ('Cardiff','South Glamorgan','Wales'),
    ('Cowbridge','South Glamorgan','Wales'),
    ('Llantwit Major','South Glamorgan','Wales'),
    ('Penarth','South Glamorgan','Wales'),
    ('Gorseinon','West Glamorgan','Wales'),
    ('Lliw Valey','West Glamorgan','Wales'),
    ('Neath','West Glamorgan','Wales'),
    ('Port Talbot','West Glamorgan','Wales'),
    ('Swansea','West Glamorgan','Wales'),
    ('Chirk','Wrexham','Wales'),
    ('Overton on Dee','Wrexham','Wales'),
    ('Rhosllannerchrugog','Wrexham','Wales'),
    ('Rhosnesni','Wrexham','Wales'),
    ('Wrexham','Wrexham','Wales');
